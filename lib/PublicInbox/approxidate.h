/*
 * approxidate stolen from libgit2 (GPL-2 w/ linking exception)
 * eb22b600633ba172791f5d4e35ee8e01a4c3525d (2025-02-26)
 */
#include <stdint.h>
#include <string.h>
#include <stdlib.h>
...
static size_t match_alpha(const char *date, struct tm *tm, int *offset)
{
	...
	for (i = 0; i < ARRAY_SIZE(timezone_names); i++) {
		...
		if (match >= 3 || match == strlen(timezone_names[i].name)) {
			...
			/*
			 * Only use the tz name offset if we don't have
			 * anything better
			 */
			if (*offset == -1)
				*offset = 60*off;
...
static int is_date(int year, int month, int day, struct tm *now_tm,
...
static size_t match_multi_number(unsigned long num, char c, const char *date,
 ...
 */
static size_t match_digit(const char *date, struct tm *tm, int *offset,
			int *tm_gmt)
{
	size_t n;
	char *end;
 ...
 */
static int match_object_header_date(const char *date, git_time_t *timestamp,
				int *offset)
{
	char *end;
	unsigned long stamp;
...
static int parse_date_basic(const char *date, git_time_t *timestamp,
			int *offset)
{
	struct tm tm;
	int tm_gmt;
...
static const char *approxidate_digit(const char *date, struct tm *tm, int *num)
{
	...
	case '-':
		if (git__isdigit(end[1])) {
			size_t match = match_multi_number(number, *end, date,
							end, tm);
			if (match)
				return date + match;
		}
...
static git_time_t approxidate_str(const char *date, time_t time_sec,
				int *error_ret)
{
	int number = 0;
	int touched = 0;
...
static int git_date_offset_parse(git_time_t *out, int *out_offset,
				const char *date)
{
	time_t time_sec;
...
static int git_date_parse(git_time_t *out, const char *date)
{
	int offset;
