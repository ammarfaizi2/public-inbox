...
=item --defrag=MSGCOUNT

Defragment SQLite and Xapian files after C<MSGCOUNT> messages indexed.
Currently only affects Linux L<btrfs(5)>, but other FSes may be supported
in the future.
